...
### Changed
- Default verifier shifted from on-chain JWT (`TwitchJWTVerifier`) to off-chain attestor with on-chain ECDSA check (`AttestorVerifier`). ~10× cheaper gas, ~3× smaller audit surface. The on-chain JWT path remains in the repo as a comparison artifact for adopters who want pure-cryptography decentralization.
...
### Added
- `TwinFactory` + `TwinAccount`: per-Twitch-user smart accounts at deterministic CREATE2 addresses, gated by on-chain JWT verification via `TwitchJWTVerifier`.
...
- On-chain JWT verifier introduction.
