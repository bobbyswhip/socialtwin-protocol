...
const TREASURY = "0xD1EC8245c8850A151843ce8a3AFdca3b19747706"; // audAdmin + keyAdmin + factory rescuer
// guardian: a DISTINCT key that can veto a pending signing-key rotation. Must
// NOT equal keyAdmin or the two-party protection collapses. Override via env.
const GUARDIAN = process.env.GUARDIAN_ADDRESS || "0xa825094B04D5a3710bd41C4fbC902F75cF333333";
...
async function main() {
  ...
  const verifier = await V.deploy([KID], [MODULUS], [OFFICIAL_CLIENT_ID], TREASURY, TREASURY, GUARDIAN);
  ...
  if (network.name !== "hardhat") {
    ...
    try {
      await run("verify:verify", { address: verifierAddr, constructorArguments: [[KID], [MODULUS], [OFFICIAL_CLIENT_ID], TREASURY, TREASURY, GUARDIAN] });
