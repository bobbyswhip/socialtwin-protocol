...
const TREASURY = "0xD1EC8245c8850A151843ce8a3AFdca3b19747706"; // audAdmin + rescuer
...
async function main() {
  ...
  const verifier = await V.deploy([KID], [MODULUS], [OFFICIAL_CLIENT_ID], TREASURY);
  ...
  if (network.name !== "hardhat") {
    ...
    try {
      await run("verify:verify", { address: verifierAddr, constructorArguments: [[KID], [MODULUS], [OFFICIAL_CLIENT_ID], TREASURY] });
